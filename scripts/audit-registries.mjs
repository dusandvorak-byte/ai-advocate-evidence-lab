...
for (const doc of documents.documents || []) {
  if (doc.institution_id && !institutionIds.has(doc.institution_id)) review.push({ type: 'missing_institution', document_id: doc.id, institution_id: doc.institution_id });
  for (const caseId of doc.case_ids || []) if (!caseIds.has(caseId)) review.push({ type: 'missing_case', document_id: doc.id, case_id: caseId });
  if (doc.public?.pdf && /^\/?web\//.test(String(doc.public.pdf))) hardErrors.push(`documents: veřejná PDF cesta obsahuje zakázaný prefix web/: ${doc.id}`);
}

for (const item of cases.cases || []) {
  if (item.institution_id && !institutionIds.has(item.institution_id)) review.push({ type: 'case_missing_institution', case_id: item.id, institution_id: item.institution_id });
  for (const related of item.related_case_ids || []) if (!caseIds.has(related)) review.push({ type: 'missing_related_case', case_id: item.id, related_case_id: related });
}

for (const deadline of deadlines.deadlines || []) {
  const documentId = deadline.document_id || deadline.source_document_id;
  if (documentId && !documentIds.has(documentId)) review.push({ type: 'deadline_missing_document', deadline_id: deadline.id || null, document_id: documentId });
  if (!deadline.source && !deadline.source_document_id && !deadline.document_id) review.push({ type: 'deadline_missing_provenance', deadline_id: deadline.id || null });
}
